...
boolean bFadeDown = false;
boolean bFadeUp = false;
MSTimer fadeTimer;
...
void updateControl() {
   ...
   simValue = getSimValue();
    ...
    boolean oldSwitchOn = bSwitchOn;
   bSwitchOn = digitalRead(switchPin);

    if( bSwitchOn != oldSwitchOn ) {
      if( bSwitchOn == false ) {
...
int updateAudio() {
  ...
  //-- check for fade down
  if( bFadeDown == true ) {
    if( fadeTimer.isExpired() == false ) {
      volMult = (float)fadeTimer.getTimeLeft()/(float)fadeTime;
     // Serial.println( volMult );
  }
    else
      bFadeDown = false;
  }
  ...
  if( bFadeUp == true) {
    if( fadeTimer.isExpired() == false ) {
      volMult = 1.0 - ((float)fadeTimer.getTimeLeft()/(float)fadeTime);
      //Serial.println( volMult );
  }
    else
      bFadeUp = false;
  ...
  if( bSwitchOn == false && bFadeDown == false )
    return 0;
